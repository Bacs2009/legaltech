$("a.an").click(function(){
	...
	$('.komentari').click(function(){$.ajax({url: "komentari.php",cache: false,beforeSend: function() {$('#mcontent').html('');},success: function(html){$("#mcontent").html(html);}});return false;});
});
...
$("input.coments").blur(function(){
	...
	$.ajax({ // описываем наш запрос
		...
		url: "/ajax", // запрос отправляем на контроллер Ajax метод addarticle
...
$("a.coments").click(function(){
	...
	$.ajax({ // описываем наш запрос
		type: "POST", // будем передавать данные через POST
		dataType: "json", // указываем, что нам вернется JSON
		url: "/ajax2", // запрос отправляем на контроллер Ajax метод addarticle
